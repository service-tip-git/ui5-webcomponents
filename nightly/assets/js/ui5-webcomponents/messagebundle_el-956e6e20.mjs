const E = "Περιεχόμενα Κάρτας", _ = "Κάρτα", T = "Κεφαλίδα Κάρτας", A = "Κεφαλίδα Διαδραστικής Κάρτας", I = "Αβαταρ", R = "{0} εμφανίστηκε, {1} κρύφθηκε.", O = "Ενεργοποίηση για ολοκληρωμένη λίστα.", L = "Μεμονωμένα άβαταρ.", N = "Συνδεδεμένα άβαταρ.", S = "Πατήστε τα πλήκτρα ΒΕΛΟΥΣ για μετακίνηση.", C = "Σήμα", D = "{0} από {1}", P = "Ιχνος Πορείας", B = "Περισσότερα", U = "Ακύρωση", t = "Περιμένετε", o = "Θετική Ενέργεια", n = "Αρνητική Ενέργεια", s = "Τονισμένο", c = "από", M = "Στοιχείο {0} του {1} εμφανίζεται", G = "Προηγούμενη Σελίδα", K = "Επόμενη Σελίδα", V = "Παλέτα Χρωμάτων - Προκαθορισμένα Χρώματα", H = "Παλέτα Χρωμάτων", X = "Χρώμα", Y = "Ακύρωση", W = "ΟΚ", F = "Αλλαγή Χρώματος", Z = "Περισσότερα Χρώματα...", e = "Έλεγχος Άλφα", r = "Ελεγχος Hue", i = "Δεκαεξαδικός", l = "Κόκκινο", a = "Πράσινο", d = "Μπλε", f = "Άλφα", u = "Ανοιγμα Επιλογέα", h = "Εισαγωγή Ημερ.", p = "Εισαγωγή Ημερομηνίας/Ώρας", w = "Είσοδος Εύρους Ημερομηνιών", J = "Διαγραφή", b = "Αναζήτηση...", g = "Φόρτωση Αρχείου", m = "Κεφαλίδα Ομάδας", x = "Πλαίσιο λίστας", j = "Επιλογή Εκδοχών", k = "Προτάσεις Διαθέσιμες", q = "Επιλογή", v = "1 αποτέλεσμα διαθέσιμο", y = "{0} αποτελέσματα είναι διαθέσιμα", z = "Χωρίς αποτελέσματα", Q = "Διακριτικό", $ = "Τονισμένο", EE = "Αναλ.γραμ.λίστας {0} από {1}", _E = "Επιλεγμένο", TE = "Δεν Επιλέχτηκε", AE = "Κεφαλίδα Ομάδας", IE = "Λειτουργία Πολλαπλής Επιλογής", RE = "Επιλογή Αναλ.Γραμ.", OE = "Περιέχει Επιλέξιμα Στοιχεία", LE = "Περιέχει Πολλά Επιλέξιμα Στοιχεία", NE = "Περιέχει Διαγράψιμα Στοιχεία", SE = "Κλείσιμο Γραμμής Πληροφοριών", CE = "Δυνατότητα κλεισίματος", DE = "Γραμμή Πληροφοριών Σφάλματος", PE = "Γραμμή Πληροφοριών Προειδοποίησης", BE = "Γραμμή Πληροφοριών Επιτυχίας", UE = "Γραμμή Πληροφοριών", tE = "ΟΚ", oE = "Αυτή η τιμή επιλέχθηκε ήδη.", nE = "Είσοδος με Πολλές Τιμές", sE = "{0} περισσότερα", cE = "Επέκταση/Σύμπτυξη", ME = "Εύρος", GE = "Αριστερός μοχλός", KE = "Δεξιός μοχλός", VE = "Αξιολόγηση", HE = "Δείκτης Αξιολόγησης", XE = "Απαιτούμενο", YE = "Απόρριψη", WE = "Ομάδα κουμπιού τμηματοποίησης", FE = "Πατήστε SPACE ή ENTER για να επιλέξετε αναλ.γραμ.", ZE = "Κουμπί τμηματοποίησης", eE = "Μοχλός ρυθμιστικού", rE = "Περισσότερα", iE = "Σειρά Κεφαλίδας 1 από {0}", lE = "{0} από {1}", aE = "Σειρά Κεφαλίδας Ομάδας", dE = "Επιλογή Αναλ.Γραμ.", fE = "Επιλέξτε Όλες τις Σειρές", uE = "Κενό", hE = "Θετική", pE = "Αρνητική", wE = "Κρίσιμη", JE = "Ουδέτερη", bE = "Καρτέλα με Υποστοιχεία", gE = "Επόμενο", mE = "Προηγούμενο", xE = "Μενού Υπερχείλισης", jE = "Περισσότερα", kE = "Ακύρωση", qE = "Πατήστε το πλήκτρο κάτω βέλους για να ανοίξετε το μενού των υποστοιχείων", vE = "{0} εναπομείναντες χαρακτήρες", yE = "{0} Πλεονάζοντες χαρακτήρες", zE = "Ωρες", QE = "Λεπτά", $E = "Δευτερόλεπτα", E_ = "ΟΚ", __ = "Ακύρωση", T_ = "Εισαγωγή Ώρας", A_ = "Πληκτρολόγηση Ώρας", I_ = "Εισαγωγή Διάρκειας", R_ = "Ημερομηνία", O_ = "Χρόνος", L_ = "Διαγράψιμο", N_ = "Δεν Υπάρχουν Διακριτικά", S_ = "Περιέχει 1 διακριτικό", C_ = "Περιέχει {0} διακριτικά", D_ = "Με Διακριτικά", P_ = "Διαγραφή", B_ = "{0} Είδη", U_ = "Αναλ.Γραμ.Δέντρου", t_ = "Επέκταση Κόμβου", o_ = "Σύμπτυξη Κόμβου", n_ = "Λάθος Κατάστασης Τιμής", s_ = "Προειδ.Κατάστ.Τιμής", c_ = "Επιτυχ.Κατάστ.Τιμής", M_ = "Πληροφορίες Κατάστασης Τιμής", G_ = "Ακυρη καταχώριση", K_ = "Προειδοποίηση εκδόθηκε", V_ = "Πληροφοριακή καταχώριση", H_ = "Καταχώριση ελέγχθηκε επιτυχώς", X_ = "Επόμενο", Y_ = "Προηγούμενο", W_ = "Αριθμός Εβδομάδας", F_ = "Μη Εργάσιμη Ημέρα", Z_ = "Σήμερα", e_ = "Επιλογέας Μήνα", r_ = "Επιλογέας Ετους", i_ = "Μείωση", l_ = "Αύξηση", a_ = "Κουμπί Διαίρεσης", d_ = "Πατήστε το πλήκτρο Διαστήματος ή Enter για να ξεκινήσει η προεπιλεγμένη ενέργεια και Alt + Κάτω Βέλος ή F4 για να ξεκινήσει η ενέργεια βέλους", f_ = "Πίσω", u_ = "Απόρριψη", h_ = "Διαδραστική Κεφαλίδα", p_ = "Πατήστε τα πλήκτρα Shift+Arrow για αλλαγή μεγέθους", w_ = "Πατήστε τα πλήκτρα Βέλους για μετακίνηση", J_ = "Πατήστε τα πλήκτρα Βέλους για μετακίνηση, τα πλήκτρα Shift+Arrow για αλλαγή μεγέθους", b_ = ":", g_ = "Additional Options", m_ = {
  ARIA_LABEL_CARD_CONTENT: E,
  ARIA_ROLEDESCRIPTION_CARD: _,
  ARIA_ROLEDESCRIPTION_CARD_HEADER: T,
  ARIA_ROLEDESCRIPTION_INTERACTIVE_CARD_HEADER: A,
  AVATAR_TOOLTIP: I,
  AVATAR_GROUP_DISPLAYED_HIDDEN_LABEL: R,
  AVATAR_GROUP_SHOW_COMPLETE_LIST_LABEL: O,
  AVATAR_GROUP_ARIA_LABEL_INDIVIDUAL: L,
  AVATAR_GROUP_ARIA_LABEL_GROUP: N,
  AVATAR_GROUP_MOVE: S,
  BADGE_DESCRIPTION: C,
  BREADCRUMB_ITEM_POS: D,
  BREADCRUMBS_ARIA_LABEL: P,
  BREADCRUMBS_OVERFLOW_ARIA_LABEL: B,
  BREADCRUMBS_CANCEL_BUTTON: U,
  BUSY_INDICATOR_TITLE: t,
  BUTTON_ARIA_TYPE_ACCEPT: o,
  BUTTON_ARIA_TYPE_REJECT: n,
  BUTTON_ARIA_TYPE_EMPHASIZED: s,
  CAROUSEL_OF_TEXT: c,
  CAROUSEL_DOT_TEXT: M,
  CAROUSEL_PREVIOUS_ARROW_TEXT: G,
  CAROUSEL_NEXT_ARROW_TEXT: K,
  COLORPALETTE_CONTAINER_LABEL: V,
  COLORPALETTE_POPOVER_TITLE: H,
  COLORPALETTE_COLOR_LABEL: X,
  COLOR_PALETTE_DIALOG_CANCEL_BUTTON: Y,
  COLOR_PALETTE_DIALOG_OK_BUTTON: W,
  COLOR_PALETTE_DIALOG_TITLE: F,
  COLOR_PALETTE_MORE_COLORS_TEXT: Z,
  COLORPICKER_ALPHA_SLIDER: e,
  COLORPICKER_HUE_SLIDER: r,
  COLORPICKER_HEX: i,
  COLORPICKER_RED: l,
  COLORPICKER_GREEN: a,
  COLORPICKER_BLUE: d,
  COLORPICKER_ALPHA: f,
  DATEPICKER_OPEN_ICON_TITLE: u,
  DATEPICKER_DATE_DESCRIPTION: h,
  DATETIME_DESCRIPTION: p,
  DATERANGE_DESCRIPTION: w,
  DELETE: J,
  FILEUPLOAD_BROWSE: b,
  FILEUPLOADER_TITLE: g,
  GROUP_HEADER_TEXT: m,
  SELECT_ROLE_DESCRIPTION: x,
  SELECT_OPTIONS: j,
  INPUT_SUGGESTIONS: k,
  INPUT_SUGGESTIONS_TITLE: q,
  INPUT_SUGGESTIONS_ONE_HIT: v,
  INPUT_SUGGESTIONS_MORE_HITS: y,
  INPUT_SUGGESTIONS_NO_HIT: z,
  LINK_SUBTLE: Q,
  LINK_EMPHASIZED: $,
  LIST_ITEM_POSITION: EE,
  LIST_ITEM_SELECTED: _E,
  LIST_ITEM_NOT_SELECTED: TE,
  LIST_ITEM_GROUP_HEADER: AE,
  ARIA_LABEL_LIST_ITEM_CHECKBOX: IE,
  ARIA_LABEL_LIST_ITEM_RADIO_BUTTON: RE,
  ARIA_LABEL_LIST_SELECTABLE: OE,
  ARIA_LABEL_LIST_MULTISELECTABLE: LE,
  ARIA_LABEL_LIST_DELETABLE: NE,
  MESSAGE_STRIP_CLOSE_BUTTON: SE,
  MESSAGE_STRIP_CLOSABLE: CE,
  MESSAGE_STRIP_ERROR: DE,
  MESSAGE_STRIP_WARNING: PE,
  MESSAGE_STRIP_SUCCESS: BE,
  MESSAGE_STRIP_INFORMATION: UE,
  MULTICOMBOBOX_DIALOG_OK_BUTTON: tE,
  VALUE_STATE_ERROR_ALREADY_SELECTED: oE,
  MULTIINPUT_ROLEDESCRIPTION_TEXT: nE,
  MULTIINPUT_SHOW_MORE_TOKENS: sE,
  PANEL_ICON: cE,
  RANGE_SLIDER_ARIA_DESCRIPTION: ME,
  RANGE_SLIDER_START_HANDLE_DESCRIPTION: GE,
  RANGE_SLIDER_END_HANDLE_DESCRIPTION: KE,
  RATING_INDICATOR_TOOLTIP_TEXT: VE,
  RATING_INDICATOR_TEXT: HE,
  RATING_INDICATOR_ARIA_DESCRIPTION: XE,
  RESPONSIVE_POPOVER_CLOSE_DIALOG_BUTTON: YE,
  SEGMENTEDBUTTON_ARIA_DESCRIPTION: WE,
  SEGMENTEDBUTTON_ARIA_DESCRIBEDBY: FE,
  SEGMENTEDBUTTONITEM_ARIA_DESCRIPTION: ZE,
  SLIDER_ARIA_DESCRIPTION: eE,
  LOAD_MORE_TEXT: rE,
  TABLE_HEADER_ROW_INFORMATION: iE,
  TABLE_ROW_POSITION: lE,
  TABLE_GROUP_ROW_ARIA_LABEL: aE,
  ARIA_LABEL_ROW_SELECTION: dE,
  ARIA_LABEL_SELECT_ALL_CHECKBOX: fE,
  ARIA_LABEL_EMPTY_CELL: uE,
  TAB_ARIA_DESIGN_POSITIVE: hE,
  TAB_ARIA_DESIGN_NEGATIVE: pE,
  TAB_ARIA_DESIGN_CRITICAL: wE,
  TAB_ARIA_DESIGN_NEUTRAL: JE,
  TAB_SPLIT_ROLE_DESCRIPTION: bE,
  TABCONTAINER_NEXT_ICON_ACC_NAME: gE,
  TABCONTAINER_PREVIOUS_ICON_ACC_NAME: mE,
  TABCONTAINER_OVERFLOW_MENU_TITLE: xE,
  TABCONTAINER_END_OVERFLOW: jE,
  TABCONTAINER_POPOVER_CANCEL_BUTTON: kE,
  TABCONTAINER_SUBTABS_DESCRIPTION: qE,
  TEXTAREA_CHARACTERS_LEFT: vE,
  TEXTAREA_CHARACTERS_EXCEEDED: yE,
  TIMEPICKER_HOURS_LABEL: zE,
  TIMEPICKER_MINUTES_LABEL: QE,
  TIMEPICKER_SECONDS_LABEL: $E,
  TIMEPICKER_SUBMIT_BUTTON: E_,
  TIMEPICKER_CANCEL_BUTTON: __,
  TIMEPICKER_INPUT_DESCRIPTION: T_,
  TIMEPICKER_CLOCK_DIAL_LABEL: A_,
  DURATION_INPUT_DESCRIPTION: I_,
  DATETIME_PICKER_DATE_BUTTON: R_,
  DATETIME_PICKER_TIME_BUTTON: O_,
  TOKEN_ARIA_DELETABLE: L_,
  TOKENIZER_ARIA_CONTAIN_TOKEN: N_,
  TOKENIZER_ARIA_CONTAIN_ONE_TOKEN: S_,
  TOKENIZER_ARIA_CONTAIN_SEVERAL_TOKENS: C_,
  TOKENIZER_ARIA_LABEL: D_,
  TOKENIZER_POPOVER_REMOVE: P_,
  TOKENIZER_SHOW_ALL_ITEMS: B_,
  TREE_ITEM_ARIA_LABEL: U_,
  TREE_ITEM_EXPAND_NODE: t_,
  TREE_ITEM_COLLAPSE_NODE: o_,
  VALUE_STATE_TYPE_ERROR: n_,
  VALUE_STATE_TYPE_WARNING: s_,
  VALUE_STATE_TYPE_SUCCESS: c_,
  VALUE_STATE_TYPE_INFORMATION: M_,
  VALUE_STATE_ERROR: G_,
  VALUE_STATE_WARNING: K_,
  VALUE_STATE_INFORMATION: V_,
  VALUE_STATE_SUCCESS: H_,
  CALENDAR_HEADER_NEXT_BUTTON: X_,
  CALENDAR_HEADER_PREVIOUS_BUTTON: Y_,
  DAY_PICKER_WEEK_NUMBER_TEXT: W_,
  DAY_PICKER_NON_WORKING_DAY: F_,
  DAY_PICKER_TODAY: Z_,
  MONTH_PICKER_DESCRIPTION: e_,
  YEAR_PICKER_DESCRIPTION: r_,
  STEPINPUT_DEC_ICON_TITLE: i_,
  STEPINPUT_INC_ICON_TITLE: l_,
  SPLIT_BUTTON_DESCRIPTION: a_,
  SPLIT_BUTTON_KEYBOARD_HINT: d_,
  MENU_BACK_BUTTON_ARIA_LABEL: f_,
  MENU_CLOSE_BUTTON_ARIA_LABEL: u_,
  DIALOG_HEADER_ARIA_ROLE_DESCRIPTION: h_,
  DIALOG_HEADER_ARIA_DESCRIBEDBY_RESIZABLE: p_,
  DIALOG_HEADER_ARIA_DESCRIBEDBY_DRAGGABLE: w_,
  DIALOG_HEADER_ARIA_DESCRIBEDBY_DRAGGABLE_RESIZABLE: J_,
  LABEL_COLON: b_,
  TOOLBAR_OVERFLOW_BUTTON_ARIA_LABEL: g_
};
export {
  E as ARIA_LABEL_CARD_CONTENT,
  uE as ARIA_LABEL_EMPTY_CELL,
  NE as ARIA_LABEL_LIST_DELETABLE,
  IE as ARIA_LABEL_LIST_ITEM_CHECKBOX,
  RE as ARIA_LABEL_LIST_ITEM_RADIO_BUTTON,
  LE as ARIA_LABEL_LIST_MULTISELECTABLE,
  OE as ARIA_LABEL_LIST_SELECTABLE,
  dE as ARIA_LABEL_ROW_SELECTION,
  fE as ARIA_LABEL_SELECT_ALL_CHECKBOX,
  _ as ARIA_ROLEDESCRIPTION_CARD,
  T as ARIA_ROLEDESCRIPTION_CARD_HEADER,
  A as ARIA_ROLEDESCRIPTION_INTERACTIVE_CARD_HEADER,
  N as AVATAR_GROUP_ARIA_LABEL_GROUP,
  L as AVATAR_GROUP_ARIA_LABEL_INDIVIDUAL,
  R as AVATAR_GROUP_DISPLAYED_HIDDEN_LABEL,
  S as AVATAR_GROUP_MOVE,
  O as AVATAR_GROUP_SHOW_COMPLETE_LIST_LABEL,
  I as AVATAR_TOOLTIP,
  C as BADGE_DESCRIPTION,
  P as BREADCRUMBS_ARIA_LABEL,
  U as BREADCRUMBS_CANCEL_BUTTON,
  B as BREADCRUMBS_OVERFLOW_ARIA_LABEL,
  D as BREADCRUMB_ITEM_POS,
  t as BUSY_INDICATOR_TITLE,
  o as BUTTON_ARIA_TYPE_ACCEPT,
  s as BUTTON_ARIA_TYPE_EMPHASIZED,
  n as BUTTON_ARIA_TYPE_REJECT,
  X_ as CALENDAR_HEADER_NEXT_BUTTON,
  Y_ as CALENDAR_HEADER_PREVIOUS_BUTTON,
  M as CAROUSEL_DOT_TEXT,
  K as CAROUSEL_NEXT_ARROW_TEXT,
  c as CAROUSEL_OF_TEXT,
  G as CAROUSEL_PREVIOUS_ARROW_TEXT,
  X as COLORPALETTE_COLOR_LABEL,
  V as COLORPALETTE_CONTAINER_LABEL,
  H as COLORPALETTE_POPOVER_TITLE,
  f as COLORPICKER_ALPHA,
  e as COLORPICKER_ALPHA_SLIDER,
  d as COLORPICKER_BLUE,
  a as COLORPICKER_GREEN,
  i as COLORPICKER_HEX,
  r as COLORPICKER_HUE_SLIDER,
  l as COLORPICKER_RED,
  Y as COLOR_PALETTE_DIALOG_CANCEL_BUTTON,
  W as COLOR_PALETTE_DIALOG_OK_BUTTON,
  F as COLOR_PALETTE_DIALOG_TITLE,
  Z as COLOR_PALETTE_MORE_COLORS_TEXT,
  h as DATEPICKER_DATE_DESCRIPTION,
  u as DATEPICKER_OPEN_ICON_TITLE,
  w as DATERANGE_DESCRIPTION,
  p as DATETIME_DESCRIPTION,
  R_ as DATETIME_PICKER_DATE_BUTTON,
  O_ as DATETIME_PICKER_TIME_BUTTON,
  F_ as DAY_PICKER_NON_WORKING_DAY,
  Z_ as DAY_PICKER_TODAY,
  W_ as DAY_PICKER_WEEK_NUMBER_TEXT,
  J as DELETE,
  w_ as DIALOG_HEADER_ARIA_DESCRIBEDBY_DRAGGABLE,
  J_ as DIALOG_HEADER_ARIA_DESCRIBEDBY_DRAGGABLE_RESIZABLE,
  p_ as DIALOG_HEADER_ARIA_DESCRIBEDBY_RESIZABLE,
  h_ as DIALOG_HEADER_ARIA_ROLE_DESCRIPTION,
  I_ as DURATION_INPUT_DESCRIPTION,
  g as FILEUPLOADER_TITLE,
  b as FILEUPLOAD_BROWSE,
  m as GROUP_HEADER_TEXT,
  k as INPUT_SUGGESTIONS,
  y as INPUT_SUGGESTIONS_MORE_HITS,
  z as INPUT_SUGGESTIONS_NO_HIT,
  v as INPUT_SUGGESTIONS_ONE_HIT,
  q as INPUT_SUGGESTIONS_TITLE,
  b_ as LABEL_COLON,
  $ as LINK_EMPHASIZED,
  Q as LINK_SUBTLE,
  AE as LIST_ITEM_GROUP_HEADER,
  TE as LIST_ITEM_NOT_SELECTED,
  EE as LIST_ITEM_POSITION,
  _E as LIST_ITEM_SELECTED,
  rE as LOAD_MORE_TEXT,
  f_ as MENU_BACK_BUTTON_ARIA_LABEL,
  u_ as MENU_CLOSE_BUTTON_ARIA_LABEL,
  CE as MESSAGE_STRIP_CLOSABLE,
  SE as MESSAGE_STRIP_CLOSE_BUTTON,
  DE as MESSAGE_STRIP_ERROR,
  UE as MESSAGE_STRIP_INFORMATION,
  BE as MESSAGE_STRIP_SUCCESS,
  PE as MESSAGE_STRIP_WARNING,
  e_ as MONTH_PICKER_DESCRIPTION,
  tE as MULTICOMBOBOX_DIALOG_OK_BUTTON,
  nE as MULTIINPUT_ROLEDESCRIPTION_TEXT,
  sE as MULTIINPUT_SHOW_MORE_TOKENS,
  cE as PANEL_ICON,
  ME as RANGE_SLIDER_ARIA_DESCRIPTION,
  KE as RANGE_SLIDER_END_HANDLE_DESCRIPTION,
  GE as RANGE_SLIDER_START_HANDLE_DESCRIPTION,
  XE as RATING_INDICATOR_ARIA_DESCRIPTION,
  HE as RATING_INDICATOR_TEXT,
  VE as RATING_INDICATOR_TOOLTIP_TEXT,
  YE as RESPONSIVE_POPOVER_CLOSE_DIALOG_BUTTON,
  ZE as SEGMENTEDBUTTONITEM_ARIA_DESCRIPTION,
  FE as SEGMENTEDBUTTON_ARIA_DESCRIBEDBY,
  WE as SEGMENTEDBUTTON_ARIA_DESCRIPTION,
  j as SELECT_OPTIONS,
  x as SELECT_ROLE_DESCRIPTION,
  eE as SLIDER_ARIA_DESCRIPTION,
  a_ as SPLIT_BUTTON_DESCRIPTION,
  d_ as SPLIT_BUTTON_KEYBOARD_HINT,
  i_ as STEPINPUT_DEC_ICON_TITLE,
  l_ as STEPINPUT_INC_ICON_TITLE,
  jE as TABCONTAINER_END_OVERFLOW,
  gE as TABCONTAINER_NEXT_ICON_ACC_NAME,
  xE as TABCONTAINER_OVERFLOW_MENU_TITLE,
  kE as TABCONTAINER_POPOVER_CANCEL_BUTTON,
  mE as TABCONTAINER_PREVIOUS_ICON_ACC_NAME,
  qE as TABCONTAINER_SUBTABS_DESCRIPTION,
  aE as TABLE_GROUP_ROW_ARIA_LABEL,
  iE as TABLE_HEADER_ROW_INFORMATION,
  lE as TABLE_ROW_POSITION,
  wE as TAB_ARIA_DESIGN_CRITICAL,
  pE as TAB_ARIA_DESIGN_NEGATIVE,
  JE as TAB_ARIA_DESIGN_NEUTRAL,
  hE as TAB_ARIA_DESIGN_POSITIVE,
  bE as TAB_SPLIT_ROLE_DESCRIPTION,
  yE as TEXTAREA_CHARACTERS_EXCEEDED,
  vE as TEXTAREA_CHARACTERS_LEFT,
  __ as TIMEPICKER_CANCEL_BUTTON,
  A_ as TIMEPICKER_CLOCK_DIAL_LABEL,
  zE as TIMEPICKER_HOURS_LABEL,
  T_ as TIMEPICKER_INPUT_DESCRIPTION,
  QE as TIMEPICKER_MINUTES_LABEL,
  $E as TIMEPICKER_SECONDS_LABEL,
  E_ as TIMEPICKER_SUBMIT_BUTTON,
  S_ as TOKENIZER_ARIA_CONTAIN_ONE_TOKEN,
  C_ as TOKENIZER_ARIA_CONTAIN_SEVERAL_TOKENS,
  N_ as TOKENIZER_ARIA_CONTAIN_TOKEN,
  D_ as TOKENIZER_ARIA_LABEL,
  P_ as TOKENIZER_POPOVER_REMOVE,
  B_ as TOKENIZER_SHOW_ALL_ITEMS,
  L_ as TOKEN_ARIA_DELETABLE,
  g_ as TOOLBAR_OVERFLOW_BUTTON_ARIA_LABEL,
  U_ as TREE_ITEM_ARIA_LABEL,
  o_ as TREE_ITEM_COLLAPSE_NODE,
  t_ as TREE_ITEM_EXPAND_NODE,
  G_ as VALUE_STATE_ERROR,
  oE as VALUE_STATE_ERROR_ALREADY_SELECTED,
  V_ as VALUE_STATE_INFORMATION,
  H_ as VALUE_STATE_SUCCESS,
  n_ as VALUE_STATE_TYPE_ERROR,
  M_ as VALUE_STATE_TYPE_INFORMATION,
  c_ as VALUE_STATE_TYPE_SUCCESS,
  s_ as VALUE_STATE_TYPE_WARNING,
  K_ as VALUE_STATE_WARNING,
  r_ as YEAR_PICKER_DESCRIPTION,
  m_ as default
};
